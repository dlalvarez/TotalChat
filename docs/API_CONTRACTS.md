...
La aprobación manual solo aplica a intentos revisables (`evidence_received`, `under_review` y `pending` para `pay_on_site`), crea `payment_reviews.decision = approved`, actualiza `payment_attempts.status = approved`, `payment_attempts.reviewed_at` y `bookings.payment_status = paid`. El rechazo requiere `reason` o `notes`, crea `payment_reviews.decision = rejected`, actualiza `payment_attempts.status = rejected`, `payment_attempts.reviewed_at` y `bookings.payment_status = rejected`. Ninguna acción confirma, cancela o libera automáticamente la cita.
