...
En Fase 7A.1.1, los fakes capturan `base_url`, una API key ficticia, modelo y timeout sin red. Fixtures mínimos cubren LLM DeepInfra (`https://api.deepinfra.com/v1/openai`, `Qwen/Qwen3.6-35B-A3B`) junto con embeddings OpenAI (`https://api.openai.com/v1`, `text-embedding-3-small`, 1536 dimensiones). Se comprueba que OpenAI y DeepInfra usan exclusivamente `TOTALCHAT_LLM_API_KEY`, que una variable específica de proveedor no tiene efecto y que la configuración recomendada no asocia `text-embedding-3-small` con DeepInfra. Otro modelo exige comprobar su dimensión. Para `reasoning_content` se prueban tres casos: exclusión por defecto, captura solo en metadata con `TOTALCHAT_LLM_CAPTURE_REASONING=true`, y respuesta sin ese atributo. En todos los casos `content` permanece como única respuesta visible y no existe persistencia ni red real. Nunca se usan tokens reales.

Las pruebas importan el contrato y el adaptador genérico, nunca módulos o clases concretas por proveedor. OpenAI y DeepInfra se construyen mediante `create_llm_provider()` y deben producir instancias de `OpenAICompatibleProvider` sin red.
