...
export type AgendaItem = { id: string; kind: 'available' | 'appointment' | 'block'; starts_at: string; ends_at: string; appointment?: Appointment; block?: AvailabilityException; isContinuation?: boolean };
...
const operationalDateTime = (value: string | Date) => (value instanceof Date ? value.toISOString() : value).slice(0, 19);
const comparableTimeMs = (value: string | Date) => { const normalized = operationalDateTime(value); const [datePart, timePart = '00:00:00'] = normalized.split('T'); const [year, month, day] = datePart.split('-').map(Number); const [hour = 0, minute = 0, second = 0] = timePart.split(':').map(Number); return Date.UTC(year, month - 1, day, hour, minute, second); };
export const belongsToSelectedDate = (startsAt: string | Date, selectedDate: string) => operationalDateTime(startsAt).slice(0, 10) === selectedDate;
export const overlapsHalfOpenRange = (eventStart: string | Date, eventEnd: string | Date, slotStart: string | Date, slotEnd: string | Date) => comparableTimeMs(eventStart) < comparableTimeMs(slotEnd) && comparableTimeMs(eventEnd) > comparableTimeMs(slotStart);
const startsInsideHalfOpenRange = (eventStart: string | Date, slotStart: string | Date, slotEnd: string | Date) => comparableTimeMs(eventStart) >= comparableTimeMs(slotStart) && comparableTimeMs(eventStart) < comparableTimeMs(slotEnd);
...
export function buildAgendaItems({ selectedDate, practitionerId, serviceId, slotMinutes, rules, appointments, blocks }: { selectedDate: string; practitionerId: string; serviceId?: string; slotMinutes: number; rules: PractitionerAvailabilityRule[]; appointments: Appointment[]; blocks: AvailabilityException[] }): AgendaItem[] {
  ...
  activeRules.forEach((r) => {
    for (let start = minutes(r.start_time); start + slotMinutes <= minutes(r.end_time); start += slotMinutes) {
      ...
      generated.set(id, { id, kind: appointment ? 'appointment' : block ? 'block' : 'available', starts_at, ends_at, appointment, block, isContinuation: appointment ? !startsInsideHalfOpenRange(appointment.starts_at, starts_at, ends_at) : undefined });
    }
  });
