...
export function buildAgendaItems({ selectedDate, practitionerId, serviceId, slotMinutes, rules, appointments, blocks }: { selectedDate: string; practitionerId: string; serviceId?: string; slotMinutes: number; rules: PractitionerAvailabilityRule[]; appointments: Appointment[]; blocks: AvailabilityException[] }): AgendaItem[] {
  ...
  const activeRules = rules.filter((r) => r.status === 'active' && r.practitioner_id === practitionerId && r.day_of_week === dayOfWeek && (!serviceId || !r.practitioner_service_id || r.practitioner_service_id === serviceId) && r.valid_from <= selectedDate && (!r.valid_to || r.valid_to >= selectedDate));
  const generated: AgendaItem[] = [];
  activeRules.forEach((r) => { for (let start = minutes(r.start_time); start + slotMinutes <= minutes(r.end_time); start += slotMinutes) { const starts_at = isoAt(selectedDate, start); const ends_at = isoAt(selectedDate, start + slotMinutes); const appointment = appointments.find((a) => a.status === 'scheduled' && overlaps(starts_at, ends_at, a.starts_at, a.ends_at)); const block = blocks.find((b) => b.status === 'active' && overlaps(starts_at, ends_at, b.starts_at, b.ends_at)); generated.push({ id: `${starts_at}-${ends_at}`, kind: appointment ? 'appointment' : block ? 'block' : 'available', starts_at, ends_at, appointment, block }); } });
  const extras: AgendaItem[] = [
    ...appointments.filter((a) => !generated.some((item) => item.appointment?.id === a.id)).map((appointment) => ({ id: `appointment-${appointment.id}`, kind: 'appointment' as const, starts_at: appointment.starts_at, ends_at: appointment.ends_at, appointment })),
    ...blocks.filter((b) => !generated.some((item) => item.block?.id === b.id)).map((block) => ({ id: `block-${block.id}`, kind: 'block' as const, starts_at: block.starts_at, ends_at: block.ends_at, block })),
  ];
  return [...generated, ...extras].sort((a, b) => a.starts_at.localeCompare(b.starts_at));
}
