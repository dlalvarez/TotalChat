...
export function OrganizationsPage({ tenant }: { tenant: AdminTenant }) {
  ...
  const disable = useMutation({ mutationFn: (org: Organization) => org.status === 'active' ? adminResourcesApi.disableOrganization(tenant.id, org.id) : adminResourcesApi.activateOrganization(tenant.id, org.id), onSuccess: async (org) => { setFeedback(org.status === 'active' ? 'Organización activada correctamente.' : 'Organización inactivada correctamente.'); await queryClient.invalidateQueries({ queryKey }); } });
  ...
  return <>
    ...
    <div className="mt-6"><SectionCard title="Listado de organizaciones" description={`Tenant activo: ${tenant.label}`}>
      ...
      {organizations.isSuccess && organizations.data.length > 0 ? <DataTableShell columns={['Nombre', 'Tipo', 'Contacto', 'Estado', 'Acciones']} rows={organizations.data.map((org) => [<NameEditButton onClick={() => startEdit(org)}>{org.name}</NameEditButton>, org.organization_type, org.email ?? org.phone ?? 'Sin contacto', <StatusBadge status={org.status} feminine />, <StatusAction status={org.status} pending={disable.isPending} onDeactivate={() => { if (window.confirm(`¿Inactivar la organización ${org.name}? No se eliminará físicamente.`)) disable.mutate(org); }} onActivate={() => { if (window.confirm(`¿Activar ${org.name} nuevamente?`)) disable.mutate(org); }} />])} /> : null}
    </SectionCard></div>
  </>;
}
