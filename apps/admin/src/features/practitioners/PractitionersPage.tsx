...
export function PractitionersPage({ tenant }: { tenant: AdminTenant }) {
  ...
  const statusAction = useMutation({ mutationFn: (p: Practitioner) => p.status === 'active' ? adminResourcesApi.disablePractitioner(tenant.id, p.id) : adminResourcesApi.activatePractitioner(tenant.id, p.id), onSuccess: async (_updated, p) => { setFeedback(p.status === 'active' ? 'Profesional inactivado correctamente.' : 'Profesional activado correctamente.'); await queryClient.invalidateQueries({ queryKey: practitionersKey }); } });
