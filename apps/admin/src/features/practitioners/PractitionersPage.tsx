...
export function PractitionersPage({ tenant }: { tenant: AdminTenant }) {
  ...
  const disable = useMutation({ mutationFn: (p: Practitioner) => p.status === 'active' ? adminResourcesApi.disablePractitioner(tenant.id, p.id) : adminResourcesApi.activatePractitioner(tenant.id, p.id), onSuccess: async (p) => { setFeedback(p.status === 'active' ? 'Profesional activado correctamente.' : 'Profesional inactivado correctamente.'); await queryClient.invalidateQueries({ queryKey: practitionersKey }); } });
